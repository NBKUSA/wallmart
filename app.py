# -*- coding: utf-8 -*-
"""app.py - Card Payment Terminal Application (Fully Functional)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1rOl7nWWLiDiCXiy2ebYQpd5UfRto7dqo
"""
...
import requests # Added for HTTP communication with ISOserver.py

# --- Firebase and Firestore Imports ---
# These are for the Canvas environment's provided Firebase setup
import firebase_admin
from firebase_admin import credentials, firestore, auth

app = Flask(__name__)
# IMPORTANT: In a real production environment, app.secret_key should be a long,
...
logging.basicConfig(level=logging.INFO) # Consider INFO for production, DEBUG for dev
...
if firebase_config:
    try:
        ...
        if initial_auth_token:
            try:
                # Sign in with custom token if provided (e.g., from Canvas)
...
# This will create the file with the desired password 'Br_3339' if it doesn't exist
# In a production environment, consider storing this password hash more securely (e.g., database).
...
if not ISO_SERVER_URL:
    ...
    ISO_SERVER_URL = 'http://127.0.0.1:5000/process_payment' # Fallback for local dev

# --- Default Payout Wallets (from environment variables) ---
# These will be used if the user doesn't specify a wallet in the payout form
...
def transactions_dashboard():
    ...
    try:
        # Fetch transactions for the current user and terminal
        transactions_ref = db.collection('artifacts').document(app_id).collection('users').document(current_user_id).collection('transactions')
        # Filter by terminal_id if you want per-terminal history or remove for all user transactions
        # query = transactions_ref.where('terminal_id', '==', session.get('terminal_id')).order_by('timestamp', direction=firestore.Query.DESCENDING).limit(100) # Example query
        query = transactions_ref.order_by('timestamp', direction=firestore.Query.DESCENDING).limit(100) # Fetch latest 100 transactions
