# app.py
from flask import Flask, render_template, request, redirect, session, url_for, flash, jsonify, g # Import g
...
logging.basicConfig(level=logging.INFO) # Keep INFO for production, but logs will show more detail now
...
# --- Add this @app.before_request to set g.user_id ---
@app.before_request
def set_global_user_id():
    """Sets the current user ID to Flask's global 'g' object for template access."""
    # current_user_id is set during Firebase initialization in app.py
...
# This would ideally be stored and tracked in Firestore per terminal ID.
# For this iteration, we'll use a simple session-based tracking (resets on app restart/session clear).
# For a robust solution for 1000 terminals, this needs a Firestore collection per terminal.
...
def login():
    if request.method == 'POST':
        ...
        if user == USERNAME and check_password(passwd):
            ...
            session['terminal_id'] = f"TERM-{os.urandom(4).hex()}" # Assign a unique terminal ID for the session
            session['daily_amount_spent'] = 0.0 # Initialize daily spending
            session['last_transaction_date'] = date.today().isoformat() # Track last transaction date
...
def protocol():
    if request.method == 'POST':
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] app.py: Protocol selected from form: '{selected}'") # DEBUG LOG
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] app.py: Session 'protocol' set to: '{session['protocol']}'") # DEBUG LOG
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] app.py: Session 'code_length' set to: {session['code_length']}") # DEBUG LOG
...
def payout():
    if request.method == 'POST':
        ...
        # Use default wallets from environment variables
        if method == 'ERC20':
        ...
        session['wallet'] = wallet # Store the chosen default wallet

        return redirect(url_for('card'))

    # Pass default wallets to the template
...
@login_required
def auth():
    # Retrieve expected_length directly from session here
    expected_length = session.get('code_length', 6)
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] app.py: Auth route - Retrieved 'code_length' from session: {expected_length}") # DEBUG LOG

    if request.method == 'POST':
        code = request.form.get('auth')
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] app.py: Auth route - Received auth code: '{code}', Length: {len(code)}") # DEBUG LOG

        if len(code) != expected_length:
            flash(f"Authorization code must be {expected_length} digits.", "error")
            return render_template('auth.html', warning=f"Code must be {expected_length} digits.", expected_length=expected_length) # Pass expected_length to template

        # Prepare data to send to ISOserver.py
        card_data_for_server = {
            'pan': session.get('pan'),
            'amount': session.get('amount'),
            'expiry': session.get('exp'),
            'cvv': session.get('cvv'),
            'currency': session.get('currency', 'USD'), # Pass selected currency
    ...
    # For GET requests to /auth, ensure expected_length is passed to the template
...
def success():
    ...
    return render_template('success.html',
        ...
        arn=session.get("arn"),
        pan=session.get("pan", "")[-4:], # Ensure only last 4 digits are passed
        amount=session.get("amount"),
...
def receipt():
    ...
    return render_template("receipt.html",
        ...
        wallet=session.get("wallet"),
        auth_code="*" * auth_digits, # This creates asterisks based on auth_digits
        iso_field_18="5999",
        iso_field_25="00",
